...
int main() {
    std::cout << "Testing pair filtering logic...\n";
    ...
    if (response.contains("result") && response["result"].contains("XXBTZUSD")) {
        ...
        std::cout << "  Pass spread filter (≤0.5%)? " << (spread_pct <= 0.5 ? "YES" : "NO") << "\n";
        std::cout << "  Pass volatility filter (>0.1%)? " << (volatility > 0.1 ? "YES" : "NO") << "\n";
        std::cout << "  Would trade? " << (spread_pct <= 0.5 && volatility > 0.1 ? "YES" : "NO") << "\n";
    } else {
        std::cout << "Failed to get ticker data\n";
    }
