...
class FairValueEngine {
    constructor(options = {}) {
        ...
        this.shortSessionEnabled = options.shortSessionEnabled === true;
        this.sessionMinTtxSeconds = Math.max(60, Number(options.sessionMinTtxSeconds || 600));
        this.sessionMaxTtxSeconds = Math.max(
            this.sessionMinTtxSeconds,
            Number(options.sessionMaxTtxSeconds || 3600)
        );
        this.shortSessionMinEdge = Number.isFinite(Number(options.shortSessionMinEdge))
            ? Math.max(0, Number(options.shortSessionMinEdge))
            : this.minEdge;
        ...
        this.categoryWeights = {
            crypto: { blackScholes: 0.25, kalshiSynthetic: 0.50, deribit: 0.25 },
    ...
    _parseContractTTXSeconds(marketId) {
        if (!marketId || typeof marketId !== 'string' || !marketId.startsWith('GEMI-')) {
            return null;
        }

        const match = marketId.match(/^GEMI-[A-Z]+(\d{10})-/);
        if (!match) return null;

        const ts = match[1]; // YYMMDDHHMM in UTC
        const year = 2000 + Number(ts.slice(0, 2));
        const month = Number(ts.slice(2, 4));
        const day = Number(ts.slice(4, 6));
        ...
        const expiryMs = Date.UTC(year, month - 1, day, hour, minute, 0, 0);
        const ttxSeconds = Math.floor((expiryMs - Date.now()) / 1000);
        return Number.isFinite(ttxSeconds) ? ttxSeconds : null;
    }

    getRequiredMinEdge(contract = {}) {
        if (!this.shortSessionEnabled) {
            return this.minEdge;
        }

        const ttxSeconds = this._parseContractTTXSeconds(contract.marketId);
        if (ttxSeconds == null) {
            return this.minEdge;
        }

        if (ttxSeconds >= this.sessionMinTtxSeconds && ttxSeconds <= this.sessionMaxTtxSeconds) {
            return this.shortSessionMinEdge;
        }
    ...
    ensembleFairValue(bsFairValue, kalshiFairValue, extras = {}) {
        ...
        if (extras.deribit != null) {
            models.push({
                value: extras.deribit,
                weight: weights.deribit || 0,
                model: { model: 'DERIBIT', fairValue: extras.deribit }
            });
        }
        ...
        // Spot-price sanity check for crypto: if spot is far from strike,
        // the probability is near 0 or 1 — reject models that disagree
        if (extras.category === 'crypto' && extras.spot && extras.strike) {
            const moneyness = extras.spot / extras.strike;
            let floorProb = null, ceilProb = null;
            if (moneyness > 1.30) floorProb = 0.90;      // spot 30%+ above strike → P(above) ≥ 90%
            else if (moneyness > 1.15) floorProb = 0.75;  // spot 15%+ above → P(above) ≥ 75%
            if (moneyness < 0.70) ceilProb = 0.10;        // spot 30%+ below strike → P(above) ≤ 10%
            else if (moneyness < 0.85) ceilProb = 0.25;   // spot 15%+ below → P(above) ≤ 25%

            if (floorProb !== null || ceilProb !== null) {
                for (const m of activeModels) {
                    if (floorProb !== null && m.value < floorProb * 0.5) {
                        // Model says P < 45% when spot says P > 90% → model is garbage
                        this.logger.debug(
                            `Sanity reject ${m.model.model}: P=${m.value.toFixed(3)} but spot/strike=${moneyness.toFixed(2)} ` +
                            `(floor=${floorProb}). Zeroing weight.`
                        );
                        m.weight = 0;
                    }
                    if (ceilProb !== null && m.value > ceilProb * 2) {
                        this.logger.debug(
                            `Sanity reject ${m.model.model}: P=${m.value.toFixed(3)} but spot/strike=${moneyness.toFixed(2)} ` +
                            `(ceil=${ceilProb}). Zeroing weight.`
                        );
                        m.weight = 0;
                    }
                }
        ...
        // Model disagreement check: if max - min > 0.40, downweight outliers
        if (activeModels.length >= 2) {
            const values = activeModels.filter(m => m.weight > 0).map(m => m.value);
            if (values.length >= 2) {
                ...
                if (maxV - minV > 0.40) {
                    ...
                    for (const m of activeModels) {
                        const deviation = Math.abs(m.value - median);
                        if (deviation > 0.30) {
                            m.weight *= 0.1; // 90% downweight for extreme outliers
                        }
                    }
                }
            }
        ...
        // Re-filter after sanity checks
        const validModels = activeModels.filter(m => m.weight > 0);
        if (validModels.length === 0) return null;

        // Normalize weights
        const totalWeight = validModels.reduce((s, m) => s + m.weight, 0);
        const ensembleValue = validModels.reduce((s, m) => s + m.value * (m.weight / totalWeight), 0);

        return {
            model: 'ENSEMBLE',
            fairValue: +ensembleValue.toFixed(4),
            components: validModels.map(m => ({
                model: m.model.model,
                fairValue: m.model.fairValue,
                weight: +(m.weight / totalWeight).toFixed(2)
            })),
            modelCount: validModels.length,
            sanitized: validModels.length < activeModels.length
        };
    }
    ...
    getBestVolatility(asset, kalshiIV = null) {
        // Hard cap: vol > 100% annualized produces garbage BS probabilities.
        // Normal BTC vol is 50-80%; Kalshi IV from thin brackets can be 3000%+.
        const MAX_VOL = 1.00;

        // Prefer Kalshi implied vol if available and confident
        if (kalshiIV && kalshiIV.impliedVol && kalshiIV.confidence >= 0.3) {
            const capped = Math.min(kalshiIV.impliedVol * this.volPremium, MAX_VOL);
            return {
                volatility: capped,
        ...
        return {
            volatility: Math.min(realizedVol, MAX_VOL),
    ...
    generateSignal(contract, kalshiAnalysis = null, kalshiIV = null, extras = {}) {
        ...
        if (bid != null && ask != null && Number(ask) <= Number(bid)) {
            return {
                actionable: false,
                marketId,
                eventTitle,
                reason: 'invalid_orderbook_crossed_or_locked',
                geminiBid: bid,
                geminiAsk: ask
            };
        }
        ...
        // Enrich extras with spot/strike for ensemble sanity checks
        const enrichedExtras = { ...extras };
        if (asset && strike) {
            enrichedExtras.strike = strike;
            const spot = this.getSpotPrice(asset);
            if (spot) enrichedExtras.spot = spot;
        }
        const ensembleFV = this.ensembleFairValue(bsFV, kalshiFV, enrichedExtras);
        ...
        // Calculate fees, spread costs, and net edge
        const feePerSide = this.feePerSide;
        const roundTripFees = entryPrice ? entryPrice * feePerSide * 2 : 0;
        const estimatedSpread = (bid !== null && ask !== null) ? Math.max(0, ask - bid) : 0.03;
        const netEdge = edge - roundTripFees - estimatedSpread;

        const requiredMinEdge = this.getRequiredMinEdge(contract);

        // Determine confidence level
        let confidence = 'low';
        if (netEdge >= this.highConfidenceEdge) confidence = 'high';
        else if (netEdge >= requiredMinEdge) confidence = 'medium';

        const actionable = netEdge >= requiredMinEdge && signal !== null;
        ...
        return {
            ...
            asset,
            strike,
            pricingModel: fairValue.model,
            direction: signal,
            entryPrice,
            edge: +edge.toFixed(4),
            netEdge: +netEdge.toFixed(4),
            roundTripFees: +roundTripFees.toFixed(6),
            requiredMinEdge: +requiredMinEdge.toFixed(4),
            confidence,
            ...
            models: {
                ...
                deribit: extras.deribit != null ? {
                    model: 'DERIBIT',
                    fairValue: extras.deribit
                } : null,
                ensemble: ensembleFV
            },
            timestamp: Date.now()
    ...
    static parseContractLabel(label) {
        const match = label.match(/(BTC|ETH|SOL|XRP|ZEC)\s*>\s*\$?([\d,]+\.?\d*)/i);
        if (!match) return null;
        return {
