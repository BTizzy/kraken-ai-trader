...
class GeminiClient {
    ...
    updatePaperMarket(marketId, referencePrice, opts = {}) {
        ...
        // Simulate wider spreads (22-27¢ as per Gemini observations)
        const spreadWidth = opts.spreadWidth || (0.22 + Math.random() * 0.05);
        // Simulated Gemini price lags reference by 0-5¢ (retail delay)
        const lag = opts.lag || (Math.random() * 0.05 - 0.025);
        const laggedPrice = Math.max(0.01, Math.min(0.99, referencePrice + lag));
        ...
        const market = {
            ...
            bid_depth: depth * 0.4,
            ask_depth: depth * 0.6,
            last_trade_time: Date.now() - Math.floor(Math.random() * 30000), // 0-30s ago
            updated: Date.now()
        };
