...
class GeminiClient {
    ...
    async refreshRealData() {
        ...
        // Full market refresh every 15s (fetches metadata + prices for all contracts)
        ...
        // Batch ticker refresh every 5s (lightweight price-only update)
        if (now - this.lastTickerFetch >= this.tickerFetchInterval) {
            try {
                await this.realClient.fetchBatchTickers('crypto');
     ...
     *
     * Auth pattern (from gemini_api_skill.md):
     *   payload = base64(JSON({ request, nonce, ...fields }))
     *   signature = HMAC-SHA384(payload, GEMINI_API_SECRET)
     *   Headers: X-GEMINI-APIKEY, X-GEMINI-PAYLOAD, X-GEMINI-SIGNATURE
     *
     * SAFETY: Only callable when mode === 'live' AND apiKey + apiSecret are set.
     */
    async _signedPost(endpoint, fields = {}) {
        if (!this.apiKey || !this.apiSecret) {
            throw new Error('HMAC auth requires GEMINI_API_KEY + GEMINI_API_SECRET in env');
        }

        const nonce = (Date.now() * 1000 + (this._nonceCounter = ((this._nonceCounter || 0) + 1) % 1000)).toString();
        const payloadObj = { request: endpoint, nonce, ...fields };
        ...
        try {
            const resp = await fetch(`${GEMINI_API}${endpoint}`, {
                method: 'POST',
                signal: controller.signal,
                headers: {
                    'Content-Type':       'application/json',
                ...
                body: payloadJson // duplicate body per Gemini docs
            });
            clearTimeout(timeout);
            ...
            if (!resp.ok) {
                ...
                if (resp.status === 429) {
                    this.logger.warn('Gemini rate limited on private endpoint');
                    await new Promise(r => setTimeout(r, 3000));
                    return this._signedPost(endpoint, fields);
                }
     ...
     * In live mode  → sends authenticated REST request with maker-or-cancel.
     *
     * @param {object} params
     *   .symbol    - instrumentSymbol (lowercase), e.g. 'btcusd-pred-above-67500-20260218-12'
     *   .side      - 'buy' | 'sell'
     *   .amount    - number of contracts (string or number)
     *   .price     - limit price (string or number)
     *   .makerOnly - bool (default true) — uses 'maker-or-cancel' option
     */
    async placeOrder(params) {
        const { symbol, side, amount, price, makerOnly = true } = params;

        // PAPER GUARD — never submit to exchange outside live mode
        if (this.mode !== 'live') {
            this.logger.debug(`[PAPER] placeOrder ${side} ${amount} ${symbol} @ ${price}`);
            return this.executePaperTrade(symbol, side === 'buy' ? 'YES' : 'NO', Number(amount));
        }
        ...
        const orderFields = {
            ...
            type: 'exchange limit',
            options: makerOnly ? ['maker-or-cancel'] : []
        };

        const result = await this._signedPost('/v1/order/new', orderFields);
        this.logger.info(
            `LIVE ORDER: ${side} ${amount} ${symbol} @ ${price} → id=${result.order_id} ` +
            `live=${result.is_live} cancelled=${result.is_cancelled}`
        );
        return result;
    }
     ...
     * @param {string} orderId - Gemini order_id
     */
    async cancelOrder(orderId) {
        ...
        return this._signedPost('/v1/order/cancel', { order_id: orderId });
    }
    ...
    async getOpenOrders() {
        ...
        return this._signedPost('/v1/orders');
    }
