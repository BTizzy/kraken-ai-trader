...
class GeminiClient {
    constructor(options = {}) {
        ...
        this.sessionMinTtxSeconds = Number.isFinite(Number(options.sessionMinTtxSeconds))
            ? Number(options.sessionMinTtxSeconds)
            : null;
        this.sessionMaxTtxSeconds = Number.isFinite(Number(options.sessionMaxTtxSeconds))
            ? Number(options.sessionMaxTtxSeconds)
            : null;
        this.sourceSessionTtxFilterEnabled = options.sourceSessionTtxFilterEnabled === true &&
            this.sessionMinTtxSeconds !== null &&
            this.sessionMaxTtxSeconds !== null;
        ...
        this.sessionUniverseStats = {
            enabled: this.sourceSessionTtxFilterEnabled,
    ...
    async refreshRealData() {
        ...
        if (this.useRealPrices) {
            ...
            for (const [key, contract] of this.realClient.contracts) {
                universeStats.total_contracts_seen += 1;
                const eligibility = this.evaluateSessionEligibility(key, now, {
                    expiryDate: contract.expiryDate,
                    resolution_date: contract.resolutionDate
                });
                if (!eligibility.allowed) {
                    universeStats.dropped_contracts += 1;
                    universeStats.by_reason[eligibility.reason] =
                        (universeStats.by_reason[eligibility.reason] || 0) + 1;
                    continue;
                }
                ...
                if (!prices) {
                    universeStats.dropped_contracts += 1;
                    universeStats.by_reason.no_prices = (universeStats.by_reason.no_prices || 0) + 1;
                    continue;
                }

                // For short-TTX mode, we allow one-sided Gemini books and rely on external
                // references (Deribit/Kalshi/BS) for fair value. Build conservative fallbacks
                // when only last trade is available so markets still flow through scoring.
                const fallbackMid = prices.lastTrade || 0.5;
                const fallbackSpread = 0.02;
                const bid = Number.isFinite(prices.bid) ? prices.bid : Math.max(0.001, fallbackMid - fallbackSpread / 2);
                const ask = Number.isFinite(prices.ask) ? prices.ask : Math.min(0.999, fallbackMid + fallbackSpread / 2);
                const spread = Number.isFinite(prices.spread) ? prices.spread : (ask - bid);
                const hasTwoSidedBook = Boolean(prices.hasTwoSidedBook && Number.isFinite(prices.bid) && Number.isFinite(prices.ask));

                universeStats.eligible_contracts += 1;
                syncedRealKeys.add(key);
                this.paperMarkets.set(key, {
                    market_id: key,
                    title: `${contract.eventTitle}: ${contract.label}`,
                    bid,
                    ask,
                    last: prices.lastTrade || (bid + ask) / 2,
                    spread,
            ...
            for (const [key, market] of Array.from(this.paperMarkets.entries())) {
                if (market?.isReal && !syncedRealKeys.has(key)) {
                    this.paperMarkets.delete(key);
                }
            }

            universeStats.synced_paper_markets = syncedRealKeys.size;
            this.sessionUniverseStats = universeStats;
        }
    }

    _getTTXSeconds(instrumentSymbol, nowMs = Date.now()) {
        if (!instrumentSymbol || !String(instrumentSymbol).startsWith('GEMI-')) return null;
        const match = String(instrumentSymbol).match(/GEMI-\w+?(\d{2})(\d{2})(\d{2})(\d{2})(\d{2})-/);
        if (!match) return null;
        const [, yy, mm, dd, hh, mn] = match;
        const expiry = new Date(`20${yy}-${mm}-${dd}T${hh}:${mn}:00Z`);
        return (expiry.getTime() - nowMs) / 1000;
    }

    _getTTXSecondsFromMeta(meta = {}, nowMs = Date.now()) {
        const expiryCandidate =
            ...
            meta.expiry ||
            null;
        if (!expiryCandidate) return null;

        const expiryMs = new Date(expiryCandidate).getTime();
        if (!Number.isFinite(expiryMs)) return null;
        return (expiryMs - nowMs) / 1000;
    }

    evaluateSessionEligibility(instrumentSymbol, nowMs = Date.now(), meta = {}) {
        const ttxFromSymbol = this._getTTXSeconds(instrumentSymbol, nowMs);
        const ttxFromMeta = this._getTTXSecondsFromMeta(meta, nowMs);
        const ttxSeconds = ttxFromSymbol !== null ? ttxFromSymbol : ttxFromMeta;

        if (!this.sourceSessionTtxFilterEnabled) {
            return { allowed: true, reason: 'disabled', ttxSeconds };
        }

        if (ttxSeconds === null) {
            return { allowed: false, reason: 'ttx_unparseable', ttxSeconds: null };
        }
        if (ttxSeconds <= 0) {
            return { allowed: false, reason: 'ttx_expired', ttxSeconds };
        }
        if (ttxSeconds < this.sessionMinTtxSeconds) {
            return { allowed: false, reason: 'ttx_below_session_min', ttxSeconds };
        }
        if (ttxSeconds > this.sessionMaxTtxSeconds) {
            if (this.allowLongTtxIn15mSession) {
                return { allowed: true, reason: 'ttx_above_session_max_allowed', ttxSeconds };
            }
            return { allowed: false, reason: 'ttx_above_session_max', ttxSeconds };
        }

        return { allowed: true, reason: 'within_session_window', ttxSeconds };
    }

    getSessionUniverseStats() {
        return {
            ...this.sessionUniverseStats,
            by_reason: { ...(this.sessionUniverseStats?.by_reason || {}) }
        };
    }
    ...
    async fetchAllActiveMarkets() {
        ...
        if (this.useRealPrices) {
            ...
            const normalized = this.realClient.getAllNormalizedMarkets()
                .filter(m => {
                    // For short-session mode, allow one-sided books and rely on
                    // external references for fair value.
                    if (m.hasTwoSidedBook) return true;
                    return Number.isFinite(m.last_price_yes);
                })
                .filter(m => this.evaluateSessionEligibility(m.market_id, Date.now(), {
                    resolution_date: m.resolution_date,
                    expiryDate: m.raw?.expiryDate || m.expiryDate
                }).allowed);
    ...
    async _signedPost(endpoint, fields = {}, _retryCount = 0) {
        ...
        const maxLeadSeconds = 20;
        if (this._lastNonce && this._lastNonce >= nowSec + maxLeadSeconds) {
            const waitMs = (this._lastNonce - (nowSec + maxLeadSeconds) + 1) * 1000;
            this.logger.warn(`Gemini nonce pacing: waiting ${waitMs}ms to stay within server window`);
        ...
        try {
            ...
            if (!resp.ok) {
                ...
                if (resp.status === 400 && /InvalidNonce/i.test(errBody) && _retryCount < 3) {
                    const serverTimeMatch = errBody.match(/server time '(\d+)'/i);
                    const serverTime = serverTimeMatch ? parseInt(serverTimeMatch[1], 10) : null;
                    if (Number.isFinite(serverTime)) {
                        this._lastNonce = serverTime;
                        const waitMs = 1500;
                        this.logger.warn(`Gemini InvalidNonce recovery: resetting nonce to server time ${serverTime} and retrying`);
                        await new Promise(r => setTimeout(r, waitMs));
                        return this._signedPost(endpoint, fields, _retryCount + 1);
                    }
                }
    ...
    getStats() {
        return {
            ...
            session_universe: this.getSessionUniverseStats()
        };
    }
