...
class GeminiPredictionsReal {
    ...
    async fetchMarkets(options = {}) {
        ...
        try {
            ...
            // Replace tracked active state for this category so stale contracts
            // do not linger across refreshes.
            const normalizedCategory = String(category || '').toLowerCase();
            for (const [ticker, event] of Array.from(this.markets.entries())) {
                if (String(event?.category || '').toLowerCase() === normalizedCategory) {
                    this.markets.delete(ticker);
                }
            }
            for (const [key, contract] of Array.from(this.contracts.entries())) {
                if (String(contract?.eventCategory || '').toLowerCase() === normalizedCategory) {
