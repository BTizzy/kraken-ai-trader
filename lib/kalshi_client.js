 ...
 *
 * Auth: RSA-PSS SHA256 signing (same scheme as KalshiWS).
...
class KalshiClient {
    constructor(options = {}) {
        ...
        // Bulk price index (like Polymarket's priceIndex) built by refreshPrices()
        // Keys: marketTicker → { bid, ask, last, spread, volume, openInterest, updated }
        this.priceIndex = new Map();

        // Load RSA private key for authenticated requests (same pattern as KalshiWS)
        ...
        if (keyPath) {
            try {
                ...
                this.logger.info('Loaded RSA private key for Kalshi REST auth');
            } catch (e) {
            ...
            try {
                ...
                this.logger.info('Loaded RSA private key from env for Kalshi REST auth');
            } catch (e) {
    ...
    /**
     * Generate RSA-PSS SHA256 auth headers for a REST request.
     * message = timestamp_ms + method + full_path (query string stripped)
     * Per Kalshi Python SDK: path_parts[0] — query params are NOT signed.
     */
    _getAuthHeaders(method, endpoint) {
        if (!this.apiKey || !this.privateKey) return {};

        const timestamp = String(Date.now());
        // Kalshi expects the full URL path (with API prefix) but WITHOUT query string
    ...
    async _fetch(endpoint, options = {}) {
        ...
        // Check cache BEFORE rate limiter — cached results shouldn't incur delay
        const cacheKey = url;
        ...
        while (retries < this.maxRetries) {
            try {
                ...
                const authHeaders = this._getAuthHeaders(method, endpoint);
     ...
     *
     * Prefers real-time WS data from bracketCache when available (< 30s old).
     * Falls back to REST orderbook fetch.
     */
    async getBestPrices(ticker) {
        // Fast path: use real-time WS data if available and fresh
        const cached = this.bracketCache.get(ticker);
        if (cached && cached.ts && (Date.now() - cached.ts) < 30000) {
            return {
                bid: cached.yesBid,
                ask: cached.yesAsk,
                last: cached.lastPrice,
                spread: (cached.yesBid != null && cached.yesAsk != null) ? cached.yesAsk - cached.yesBid : null,
    ...
    normalizeMarket(rawMarket) {
        // Determine category — prefer Kalshi's own API category field, fall back to keywords
        let category = 'other';
        const apiCategory = (rawMarket.category || '').toLowerCase().trim();

        if (apiCategory && apiCategory !== 'other') {
            // Normalize Kalshi API categories to our internal names
            const categoryMap = {
                'politics': 'politics',
                'economics': 'economics',
                'finance': 'finance',
            ...
            category = categoryMap[apiCategory] || apiCategory;
        } else {
            // Fallback: keyword detection from title/series_ticker
            ...
            if (title.includes('election') || title.includes('president') || title.includes('congress') ||
                title.includes('senate') || title.includes('governor') || title.includes('vote') ||
                title.includes('legislation') || title.includes('tariff') || title.includes('government') ||
                ...
                category = 'politics';
                       ...
                       title.includes('crypto') || title.includes('solana') || title.includes('xrp') ||
                       seriesTicker.includes('btc') || seriesTicker.includes('eth') || seriesTicker.includes('sol')) {
                category = 'crypto';
            } else if (title.includes('super bowl') || title.includes('nfl') || title.includes('nba') ||
                       title.includes('mlb') || title.includes('nhl') || title.includes('soccer') ||
                       title.includes('tennis') || title.includes('ufc') || title.includes('mma') ||
                       title.includes('hockey') || title.includes('college') || title.includes('march madness') ||
                       title.includes('sports') || title.includes('world series') || title.includes('stanley cup') ||
                       title.includes('championship')) {
                category = 'sports';
            } else if (title.includes('fed') || title.includes('gdp') || title.includes('inflation') ||
                       title.includes('cpi') || title.includes('rate') || title.includes('jobs') ||
                       title.includes('unemployment') || title.includes('housing') || title.includes('trade deficit') ||
                       title.includes('treasury') || title.includes('s&p') || title.includes('nasdaq') ||
                       title.includes('stock market')) {
                category = 'economics';
            } else if (title.includes('ai ') || title.includes('artificial intelligence') || title.includes('chatgpt') ||
                       title.includes('openai') || title.includes('google') || title.includes('apple') ||
                       title.includes('spacex') || title.includes('tesla') || title.includes('tech')) {
                category = 'tech';
            } else if (title.includes('oscar') || title.includes('grammy') || title.includes('emmy') ||
                       title.includes('movie') || title.includes('album') || title.includes('celebrity') ||
                       title.includes('reality tv') || title.includes('entertainment')) {
                category = 'culture';
            }
        }
    ...
    getStats() {
        return {
            ...
            price_index_size: this.priceIndex.size,
            bracket_cache_size: this.bracketCache.size,
    ...
    /**
     * Bulk-refresh prices for all active Kalshi markets.
     * Builds an in-memory priceIndex (mirrors Polymarket's refreshPrices pattern).
     * The /markets endpoint returns yes_bid, yes_ask, last_price inline — no
     * separate orderbook call needed per market.
    ...
    async refreshPrices() {
        try {
            ...
            for (const m of markets) {
                ...
                const yesBid = m.yes_bid != null ? m.yes_bid / 100 : null;
                const yesAsk = m.yes_ask != null ? m.yes_ask / 100 : null;
                const lastPrice = m.last_price != null ? m.last_price / 100 : null;

                if (yesBid == null && yesAsk == null && lastPrice == null) continue;

                this.priceIndex.set(ticker, {
                    bid: yesBid,
                    ask: yesAsk,
                    last: lastPrice,
                    spread: (yesBid != null && yesAsk != null) ? yesAsk - yesBid : null,
            ...
            this.logger.debug(`Kalshi price index updated: ${this.priceIndex.size} markets`);
            return this.priceIndex.size;
        } catch (error) {
            this.logger.warn('Kalshi price refresh failed: ' + error.message);
            return 0;
        }
    }

    /**
     * Get cached price for a market ticker from the bulk priceIndex.
     * Returns { bid, ask, last, spread, volume, openInterest } or null.
     */
    getCachedPrice(ticker) {
        if (!ticker) return null;
        const cached = this.priceIndex.get(ticker);
        if (!cached) return null;
        if (Date.now() - cached.updated > 60000) return null;
        return cached;
    }
    ...
    async getBracketsByEvent(asset) {
        ...
        // Check bracket cache (60s TTL) — brackets change slowly, no need to refetch every cycle
        const bracketCacheKey = `brackets_${asset}`;
        const cachedBrackets = this._bracketResultCache?.get(bracketCacheKey);
        if (cachedBrackets && (Date.now() - cachedBrackets.time) < 60000) {
            return cachedBrackets.data;
        }
        ...
        // Cache bracket results (60s TTL)
        if (!this._bracketResultCache) this._bracketResultCache = new Map();
        this._bracketResultCache.set(bracketCacheKey, { data: events, time: Date.now() });
    ...
    computeSyntheticAbove(brackets) {
        ...
        for (let i = 0; i < between.length; i++) {
            ...
            // Filter out illiquid brackets: spread > 0.50 or no volume
            // These produce garbage synthetic probabilities
            const liquid = remaining.filter(b => {
                const spread = Math.abs((b.yesAsk || 0) - (b.yesBid || 0));
                return spread <= 0.50 && (b.volume > 0 || b.hasLiquidity);
            });
            // Use liquid brackets for price, but track total for context
            const useBrackets = liquid.length > 0 ? liquid : remaining;
            const rawMid = useBrackets.reduce((s, b) => s + b.mid, 0);
            const rawBid = useBrackets.reduce((s, b) => s + b.yesBid, 0);
            const rawAsk = useBrackets.reduce((s, b) => s + b.yesAsk, 0);
            result[between[i].floorStrike] = {
                mid: Math.min(1, Math.max(0, rawMid)),
                bidSum: Math.min(1, Math.max(0, rawBid)),
                askSum: Math.min(1, Math.max(0, rawAsk)),
                bracketCount: remaining.length,
                liquidBrackets: liquid.length,
