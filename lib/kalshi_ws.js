 ...
 *
 * Real-time bracket price updates via Kalshi Fast API WebSocket.
 *
 ...
 *   const ws = new KalshiWS({ apiKey: process.env.KALSHI_API_KEY });
...
class KalshiWS extends EventEmitter {
    ...
    async connect() {
        ...
        // Without an API key, Kalshi WS returns 401 — operate without real-time data
        if (!this.apiKey) {
            this.logger.info('No KALSHI_API_KEY — Kalshi WS disabled (REST polling active)');
            return;
        }
    ...
    /**
     * Subscribe to ticker_v2 for an array of market tickers.
    ...
    unsubscribe(marketTickers) {
        ...
        if (this.connected && this.ws?.readyState === WebSocket.OPEN) {
            this.ws.send(JSON.stringify({
                id:     this.requestId++,
                cmd:    'unsubscribe',
                params: { channels: ['ticker_v2'], market_tickers: marketTickers }
            }));
        }
    }
