...
class MarketMatcher {
    ...
    async runMatchCycle(polyClient, kalshiClient, geminiClient) {
        ...
        try {
            ...
            // Step 3b: Add unmatched GEMI-* crypto contracts for FV-only pricing
            // Contracts for assets without Kalshi bracket matches (XRP, ZEC, etc.)
            // or SOL contracts where Kalshi brackets were empty still need to be
            // in the match list so the fair value engine can price them via Black-Scholes.
            const matchedGemiIds = new Set(matches.map(m => m.gemini_market_id));
            const unmatchedGemi = effectiveGemini.filter(m =>
                m.market_id && m.market_id.startsWith('GEMI-') &&
                m.category === 'crypto' &&
                !matchedGemiIds.has(m.market_id)
            );
            for (const contract of unmatchedGemi) {
                matches.push({
                    ...
                    manual_override: false,
                    crypto_match: true,
                    fv_only: true  // No cross-platform reference, BS fair value only
                });
            }
