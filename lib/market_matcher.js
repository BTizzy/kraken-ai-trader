...
class MarketMatcher {
    ...
    createSimulatedGeminiMarkets(polyMarkets, kalshiMarkets, geminiClient) {
        ...
        // Take top 30 Polymarket markets by volume (most likely to be on Gemini)
        ...
        for (const poly of topPoly) {
            ...
            geminiClient.updatePaperMarket(geminiId, refPrice, {
                title: poly.title,
                volume: Math.floor((poly.volume || 10000) * 0.05), // Gemini has ~5% of Polymarket volume
            });
            ...
            simulated.push({
                ...
                volume: Math.floor((poly.volume || 10000) * 0.05),
