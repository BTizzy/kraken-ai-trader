...
class PaperTradingEngine {
    ...
    async enterPosition(signal) {
        ...
        // Spread-aware entry: edge must exceed the actual Gemini spread
        // Otherwise the trade is underwater immediately after entry
        ...
        if (geminiSpread && geminiSpread > 0) {
            ...
            if (refPrice) {
                ...
                const minRequiredEdge = Math.max(this.params.stop_loss_width || 0.03, geminiSpread * 1.2);
                if (edgeEst < minRequiredEdge) {
                    this.logger.debug(
                        `Spread-aware reject: ${signal.marketId} edge=${edgeEst.toFixed(3)} < ` +
                        `minRequired=${minRequiredEdge.toFixed(3)} (spread=${geminiSpread.toFixed(3)})`
                    );
                    return null;
                }
            }
        ...
        if ((this.gemini.mode === 'live' || this.gemini.mode === 'sandbox') && isRealInstrument) {
            ...
            // Minimum balance check: need at least min position + $2 buffer
            if (wallet.balance < (this.params.min_position_size || 5) + 2) {
                this.logger.warn(
                    `Live order rejected: insufficient balance $${wallet.balance.toFixed(2)} ` +
            ...
            try {
                ...
                this.logger.error(`Live order failed for ${signal.marketId}: ${err.message}`);
                // Fall back to paper trade so we can track signals and learn
                    ...
                    try {
                        ...
                        if (!exitOrder || !exitOrder.success) {
                            this.logger.warn(
                                ...
                                `will retry next cycle (reason=${exitReason})`
                            );
        ...
        try {
            ...
            if (winRate < 0.50) {
                ...
                if (this._tighteningStreak >= 5 && this.params.entry_threshold >= 55) {
                    const newThreshold = Math.max(45, this.params.entry_threshold * 0.9);
                    this.db.setParameter('entry_threshold', newThreshold);
                    this.params.entry_threshold = newThreshold;
                    this._tighteningStreak = 0;
