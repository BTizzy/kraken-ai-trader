...
class PaperTradingEngine {
    constructor(db, geminiClient, options = {}) {
        ...
        this.liveMinTradableBalance = Math.max(0.5, Number(options.liveMinTradableBalance ?? process.env.LIVE_MIN_TRADABLE_BALANCE ?? 2));
    ...
    getTradableLiveBalance(balance) {
        ...
        return Math.max(0, numericBalance - this.liveUsdReserve);
    }

    hasTradableLiveBalance(balance) {
        const tradable = this.getTradableLiveBalance(balance);
        return Number.isFinite(tradable) && tradable >= this.liveMinTradableBalance;
    }
    ...
    getReadinessBlockers() {
        ...
        if (this.autonomous15mSession) {
            ...
            if (isLiveMode && Number.isFinite(Number(this._liveBalance))) {
                ...
                if (!Number.isFinite(tradable) || tradable < this.liveMinTradableBalance) {
                    blockers.push(`live_balance:below_reserve_floor_${this.liveMinTradableBalance}`);
                }
            }
        }
    ...
    async evaluatePreTradeSafetyGate(force = false) {
        ...
        try {
            ...
            if (isLiveMode && (!Number.isFinite(tradableBalance) || tradableBalance < this.liveMinTradableBalance)) {
                this._lastPreTradeGate = {
                    ts: now,
                    allowed: false,
    ...
    async enterPosition(signal) {
        ...
        if (isLive) {
            ...
            if (!this.hasTradableLiveBalance(this._liveBalance)) {
                this.logEntryRejection(
                    signal,
                    'live_balance',
                    `tradable_${Number(this._liveTradableBalance || 0).toFixed(2)}_lt_${this.liveMinTradableBalance.toFixed(2)}`,
                    {
        ...
        if ((this.gemini.mode === 'live' || this.gemini.mode === 'sandbox') && isRealInstrument) {
            ...
            if (!Number.isFinite(tradableBalance) || tradableBalance < this.liveMinTradableBalance) {
                if (!this._balanceRejectLogged) {
                    this.logger.warn(
                        ...
                        `need >= $${this.liveMinTradableBalance.toFixed(2)})` +
                ...
                this.logEntryRejection(
                    ...
                    `tradable_balance_${Number(tradableBalance || 0).toFixed(2)}_lt_${this.liveMinTradableBalance.toFixed(2)}`,
                    {
                        entryPrice,
                        totalBalance,
                        tradableBalance,
    ...
    async monitorPositions() {
        ...
        for (const trade of openTrades) {
            ...
            if (isLive) {
                ...
                currentMid = realPrices ? (realPrices.bid + realPrices.ask) / 2 : (trade.entry_price + 0.01);
                ...
                exitPrice = realPrices && trade.direction === 'YES' ? realPrices.bid : (realPrices && trade.direction === 'NO' ? (1 - realPrices.ask) : null);
            } else {
