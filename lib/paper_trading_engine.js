        ...
        if ((this.gemini.mode === 'live' || this.gemini.mode === 'sandbox') && isRealInstrument) {
            ...
            if (this.gemini.realClient) {
                ...
                if (realPrices.spread > 0.15) {
                    ...
                    this.logEntryRejection(signal, 'liquidity_gate', `spread_${realPrices.spread.toFixed(3)}_gt_0.15`, { entryPrice });
                    return null;
                }
            }
    ...
    async monitorPositions() {
        ...
        for (const trade of openTrades) {
            ...
            if (expiryMatch) {
                ...
                timeToExpiry = (expiry.getTime() - Date.now()) / 1000;
                if (timeToExpiry > 0) {
            ...
            if (!exitReason && timeToExpiry !== null && timeToExpiry > 0 && timeToExpiry < 300) {
                // Less than 5 minutes to settlement — force exit
                    ...
                    try {
                        ...
                        if (!filled) {
                            this.logger.warn(
                                ...
                                `no position or active order found on exchange. May have been cancelled.`
                            );
                        }
                    } catch (histErr) {
