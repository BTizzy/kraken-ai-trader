...
class PaperTradingEngine {
    ...
    getRequiredSessionRemainingSeconds() {
        return Math.max(90, Math.min(this.getMinEntryTtxSeconds(), 300));
    }
    ...
    async enterPosition(signal) {
        ...
        if (this.autonomous15mSession && signal.marketId && signal.marketId.startsWith('GEMI-')) {
            ...
            const scoreFloor = Math.max(this.params.entry_threshold || 45, 40);
            ...
            // Keep a small absolute floor, but let min_edge_live drive session tuning.
            const edgeFloor = Math.max(this.params.min_edge_live || 0.05, 0.02);
