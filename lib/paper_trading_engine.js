...
class PaperTradingEngine {
    constructor(db, geminiClient, options = {}) {
        ...
        this.preTradeGateCacheMs = Math.max(1000, Number(options.preTradeGateCacheMs ?? process.env.PRE_TRADE_GATE_CACHE_MS ?? 10000));
    ...
    markSessionStart(liveDailyPnl = null) {
    ...
    _isEligible15mSessionMarket(marketId) {
        ...
        return ttx >= this.sessionMinTtxSeconds && ttx <= this.sessionMaxTtxSeconds;
    }
        ...
        try {
            ...
            if (!isLiveMode) {
                this._lastPreTradeGate = {
                    ts: now,
                    allowed: false,
                    reason: 'session_requires_live_mode',
                    details: { mode: this.gemini.mode }
                };
                return this._lastPreTradeGate;
            }

            const [recon, openOrders, liveDaily] = await Promise.all([
                this.reconcilePositions(),
                this.gemini.getOpenOrders(),
                Promise.resolve(this.db.getDailyPnL('live'))
            ]);
    ...
    normalizeTradingProfile(profile) {
        ...
        if (['short', 'short-run', 'session', 'session-run'].includes(normalized)) {
            return 'short-run';
        }
        return 'standard';
    }
    ...
    getTradingProfileOverrides(profile) {
        if (profile === 'short-run') {
            return {
                ...
                max_hold_time: 1800,
                high_score_min_hold_time: 1800,
                pre_expiry_exit_seconds: 1800,
                time_decay_start_fraction: 0.65
            };
        }
    ...
    async _hasExchangePosition(symbol) {
        try {
            ...
            return (positions || []).some(p => {
                ...
                const onHold = Number(p.quantityOnHold || 0);
                return (total - onHold) > 0;
            });
        } catch (err) {
    ...
    calculatePositionSize(signal, wallet) {
        ...
        const maxPerTrade = isLive
            ? Math.min(liveMaxPos, effectiveBalance * 0.10)
    ...
    canEnterPosition(signal) {
        ...
        if (this.autonomous15mSession && isLiveSignal) {
            if (!this._isEligible15mSessionMarket(signal.marketId)) {
                return {
                    allowed: false,
                    reason: `15m_session_ttx_filter (allowed ${this.sessionMinTtxSeconds}-${this.sessionMaxTtxSeconds}s)`
                };
            }
        }
        ...
        const minEntryTTX = (this.params.pre_expiry_exit_seconds || 300) + 300; // pre_expiry + 5min buffer
    ...
    async enterPosition(signal) {
        ...
        if (this.autonomous15mSession && signal.marketId && signal.marketId.startsWith('GEMI-')) {
            ...
            const scoreFloor = Math.max(this.params.entry_threshold || 45, 60);
            ...
            const edgeFloor = Math.max(this.params.min_edge_live || 0.05, 0.08);
        ...
        if ((this.gemini.mode === 'live' || this.gemini.mode === 'sandbox') && isRealInstrument) {
            ...
            if (this.gemini.realClient) {
                ...
                if (realPrices.spread > 0.08) {
                    this.logger.debug(
                        `Liquidity gate: ${signal.marketId} — spread=$${realPrices.spread.toFixed(3)} > $0.08`
                    );
                    this.logEntryRejection(signal, 'liquidity_gate', `spread_${realPrices.spread.toFixed(3)}_gt_0.08`, { entryPrice });
                    return null;
                }
            }

            // Minimum edge for live trades (lower bar for short-TTX contracts)
            const shortTTXEntry = this._isShortTTX(signal.marketId);
            const minEdgeLive = shortTTXEntry ? 0.03 : (this.params.min_edge_live || 0.08);
            ...
            if (signalEdge < minEdgeLive) {
                ...
                this.logEntryRejection(signal, 'live_min_edge', `edge_${signalEdge.toFixed(3)}_lt_${minEdgeLive}`, { entryPrice, edge: signalEdge });
                return null;
            }
            ...
            if (effectiveBalance < (this.params.min_position_size || 5) + 2) {
                ...
                this.logEntryRejection(signal, 'insufficient_balance', `balance_${effectiveBalance.toFixed(2)}`, { entryPrice });
                return null;
            }
    ...
    async monitorPositions() {
        ...
        for (const trade of openTrades) {
            ...
            if (expiryCheck.timeToExpiry !== null && expiryCheck.timeToExpiry <= 0) {
                ...
                // Determine settlement value: use last known direction to infer outcome
                // For YES contracts settled ITM → $1, OTM → $0. Vice versa for NO.
                const exitPrice = trade.direction === 'YES' ? 0.0 : 1.0; // assume worst case
                const pnl = parseFloat(
                    ((exitPrice - trade.entry_price) * trade.position_size / trade.entry_price).toFixed(4)
                );
                this.logger.info(
                    `EXPIRED SETTLEMENT: ${trade.gemini_market_id} — ` +
                    `${Math.round(-expiryCheck.timeToExpiry)}s past settlement, force-closing (assumed ${exitPrice === 1.0 ? 'WIN' : 'LOSS'})`
                );
            ...
            if (isLive) {
                ...
                const spread = realPrices.ask - realPrices.bid;
            ...
            const preExpiryExitSeconds = this.params.pre_expiry_exit_seconds || 300;
        ...
        try {
            ...
            for (const trade of dbOpenTrades) {
                ...
                if (exchangePos) {
                    ...
                    const dbContracts = trade.entry_price > 0
                        ? Math.round(trade.position_size / trade.entry_price)
                        : 0;
                    ...
                    if (hasMismatch) {
                        ...
                        this.logger.warn(
                            `QTY MISMATCH: trade ${trade.id} ${trade.direction} ${trade.gemini_market_id} ` +
                    ...
                    this.logger.warn(
                        ...
                        (hasPendingExit ? ' (exit in-flight, may resolve next cycle)' : '')
                    );
                }
            }
