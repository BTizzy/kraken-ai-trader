...
class PaperTradingEngine {
    ...
    loadParams() {
        const defaults = {
            ...
            daily_loss_limit: -50,
            // Fee model: Gemini Predictions taker fees per side (entry + exit)
            // From JS bundle analysis of takerFee + platformFee fields
            // Conservative estimate: 2% per side
            fee_per_side: 0.02
        };
    ...
    calculatePositionSize(signal, wallet) {
        ...
        // Expected edge = price gap between Gemini and reference (Polymarket) price
        let edge;
        if (signal.direction === 'YES') {
            // Buying YES: our edge is referencePrice - entryPrice (Gemini is below Polymarket)
            edge = referencePrice ? (referencePrice - entryPrice) : 0;
        } else {
            // Selling YES (buying NO): our edge is entryPrice - referencePrice (Gemini is above Polymarket)
    ...
    monitorPositions() {
        ...
        for (const trade of openTrades) {
            ...
            //   Stop loss: based on mid-price (standard practice — spread cost isn't a loss trigger)
            if (trade.direction === 'YES') {
                if (exitPrice >= trade.take_profit_price) exitReason = 'take_profit';
                else if (currentMid <= trade.stop_loss_price) exitReason = 'stop_loss';
            } else {
                if (exitPrice <= trade.take_profit_price) exitReason = 'take_profit';
                else if (currentMid >= trade.stop_loss_price) exitReason = 'stop_loss';
            }
