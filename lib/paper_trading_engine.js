...
class PaperTradingEngine {
    constructor(db, geminiClient, options = {}) {
        ...
        this.dataMinedRules = this.loadDataMinedRules(
            options.dataMinedRulesPath || process.env.DATA_MINED_RULES_PATH
        );
        ...
        this.liveCanaryMinPositionSizeUsd = Math.max(
            0.01,
            Number(options.liveCanaryMinPositionSizeUsd ?? process.env.LIVE_CANARY_MIN_POSITION_SIZE_USD ?? 0.25)
        );
        ...
        this.allowSuspendedLiveCanary = String(
            options.allowSuspendedLiveCanary ?? process.env.ALLOW_SUSPENDED_LIVE_CANARY ?? ''
        ).toLowerCase() === 'true';
        this.maxSuspendedCanaryPosUsd = Math.max(
            0.01,
            Number(options.maxSuspendedCanaryPosUsd ?? process.env.MAX_SUSPENDED_CANARY_POS_USD ?? 0.10)
        );
        this.liveMinEntryPrice = Math.max(
            0.01,
            Number(options.liveMinEntryPrice ?? process.env.LIVE_MIN_ENTRY_PRICE ?? 0.10)
        );
        this.liveCanaryMinEntryPrice = Math.max(
            0.01,
            Number(options.liveCanaryMinEntryPrice ?? process.env.LIVE_CANARY_MIN_ENTRY_PRICE ?? 0.05)
        );
        this.liveMinScore = Math.max(
            0,
            Number(options.liveMinScore ?? process.env.LIVE_MIN_SCORE ?? 45)
        );
        this.liveCanaryMinScore = Math.max(
            0,
            Number(options.liveCanaryMinScore ?? process.env.LIVE_CANARY_MIN_SCORE ?? 30)
        );
        this.noLeverageMinEntryPrice = Math.max(
            0.01,
            Number(options.noLeverageMinEntryPrice ?? process.env.NO_LEVERAGE_MIN_ENTRY_PRICE ?? 0.05)
        );
        this.liveCanaryNoLeverageMinEntryPrice = Math.max(
            0.01,
            Number(options.liveCanaryNoLeverageMinEntryPrice ?? process.env.LIVE_CANARY_NO_LEVERAGE_MIN_ENTRY_PRICE ?? 0.03)
        );
        ...
        this.liveDefaultMinPositionSizeUsd = Math.max(0.01, Number(
            options.liveDefaultMinPositionSizeUsd
            ?? process.env.LIVE_DEFAULT_MIN_POSITION_SIZE_USD
            ?? 1.00
        ));
        this.liveLowBalanceMinPositionSizeUsd = Math.max(0.01, Number(
            options.liveLowBalanceMinPositionSizeUsd
            ?? process.env.LIVE_LOW_BALANCE_MIN_POSITION_SIZE_USD
            ?? 0.10
        ));
        this.liveAdministrativeExitMaxSpread = Math.max(0.01, Number(
            options.liveAdministrativeExitMaxSpread
            ?? process.env.LIVE_ADMIN_EXIT_MAX_SPREAD
            ?? 0.08
        ));
        this.liveAdministrativeExitMinProfit = Math.max(0, Number(
            options.liveAdministrativeExitMinProfit
            ?? process.env.LIVE_ADMIN_EXIT_MIN_PROFIT
            ?? 0.01
        ));
    ...
    loadDataMinedRules(customPath) {
        const fallbackPath = path.join(__dirname, '..', 'config', 'data_mined_strategy_rules.json');
        const rulesPath = customPath || fallbackPath;
        try {
            if (!fs.existsSync(rulesPath)) return null;
            const parsed = JSON.parse(fs.readFileSync(rulesPath, 'utf8'));
            return {
                min_entry_score_recommended: Number(parsed.min_entry_score_recommended || 0),
                direction_bias: parsed.direction_bias || 'neutral',
                avoid_spread_buckets: Array.isArray(parsed.avoid_spread_buckets) ? parsed.avoid_spread_buckets : [],
                avoid_score_buckets: Array.isArray(parsed.avoid_score_buckets) ? parsed.avoid_score_buckets : [],
                avoid_categories: Array.isArray(parsed.avoid_categories) ? parsed.avoid_categories : [],
                suspend_live_entries: parsed.suspend_live_entries === true
            };
        } catch (error) {
            this.logger.warn(`Unable to load data-mined rules: ${error.message}`);
            return null;
        }
    }
    ...
    applyDataMinedRuleGate(signal) {
        if (!this.dataMinedRules) return { allowed: true };

        const rules = this.dataMinedRules;
        const isLiveMode = this.gemini.mode === 'live' || this.gemini.mode === 'sandbox';
        const canaryBypass = this.allowSuspendedLiveCanary
            && this.liveCanaryMode
            && this.liveCanaryMaxPositionSizeUsd <= this.maxSuspendedCanaryPosUsd;
        const score = Number(signal.score || 0);
        const scoreBucket = this._bucketScore(score);
        const category = String(signal.category || 'unknown');
        const spread = Number(signal.geminiSpread || ((signal.gemini_ask != null && signal.gemini_bid != null)
            ? Number(signal.gemini_ask) - Number(signal.gemini_bid)
            : NaN));
        const spreadBucket = this._bucketSpread(spread);

        if (isLiveMode && rules.suspend_live_entries && !canaryBypass) {
            return { allowed: false, reason: 'data_mined_suspend_live_entries' };
        }

        // In canary bootstrap mode we only bypass the mined score/spread/category filters.
        // Direction bias still applies to keep directional risk constraints active.
        if (!canaryBypass) {
            if (rules.min_entry_score_recommended > 0 && score < rules.min_entry_score_recommended) {
                return { allowed: false, reason: `data_mined_score_${score}_lt_${rules.min_entry_score_recommended}` };
            }
            if (rules.avoid_score_buckets.includes(scoreBucket)) {
                return { allowed: false, reason: `data_mined_avoid_score_bucket:${scoreBucket}` };
            }
            if (rules.avoid_spread_buckets.includes(spreadBucket)) {
                return { allowed: false, reason: `data_mined_avoid_spread_bucket:${spreadBucket}` };
            }
            if (rules.avoid_categories.includes(category)) {
                return { allowed: false, reason: `data_mined_avoid_category:${category}` };
            }
        }
        if (rules.direction_bias === 'prefer_no' && signal.direction === 'YES') {
            return { allowed: false, reason: 'data_mined_direction_bias:prefer_no' };
        }
        if (rules.direction_bias === 'prefer_yes' && signal.direction === 'NO') {
            return { allowed: false, reason: 'data_mined_direction_bias:prefer_yes' };
        }
    ...
    getLiveMinPositionSizeUsd(balance) {
        const numericBalance = Number(balance);
        if (!Number.isFinite(numericBalance) || numericBalance <= 0) {
            return this.liveDefaultMinPositionSizeUsd;
        }

        if (this.liveCanaryMode) {
            return Math.min(this.liveCanaryMinPositionSizeUsd, numericBalance * 0.90);
        }

        if (numericBalance <= this.liveLowBalanceSizingThresholdUsd) {
            return Math.min(this.liveLowBalanceMinPositionSizeUsd, numericBalance * 0.90);
        }

        return Math.min(this.liveDefaultMinPositionSizeUsd, numericBalance * 0.90);
    }
    ...
    getSessionPolicy() {
        ...
        return {
            ...
            live_default_min_position_size_usd: this.liveDefaultMinPositionSizeUsd,
            live_low_balance_min_position_size_usd: this.liveLowBalanceMinPositionSizeUsd,
            data_mined_rules_active: !!this.dataMinedRules,
            data_mined_rules: this.dataMinedRules,
            ...
            allow_suspended_live_canary: this.allowSuspendedLiveCanary,
            max_suspended_canary_pos_usd: this.maxSuspendedCanaryPosUsd,
    ...
    async evaluatePreTradeSafetyGate(force = false) {
        ...
        try {
            ...
            if (isLiveMode) {
                this._liveBalance = Number.isFinite(currentBalance) ? currentBalance : null;
                this._liveTradableBalance = Number.isFinite(currentBalance)
    ...
    getMinEntryTtxSeconds() {
        const preExpiryExitSeconds = Number(this.params.pre_expiry_exit_seconds || 300);
        const effectivePreExpiryExit = this.liveCanaryMode
            ? Math.min(preExpiryExitSeconds, 60)
            : preExpiryExitSeconds;
        return effectivePreExpiryExit + this.sessionEntryBufferSeconds;
    }
    ...
    _getAdministrativeLiveExitFloor(trade) {
        const entryPrice = Number(trade?.entry_price || 0);
        if (!Number.isFinite(entryPrice) || entryPrice <= 0) return null;
        return Math.min(0.99, entryPrice + this.liveAdministrativeExitMinProfit);
    }

    _shouldDeferAdministrativeLiveExit({ trade, exitReason, exitPrice, pnl, timeToExpiry, hasTwoSidedBook, spread }) {
        if (!['time_exit', 'pre_expiry_exit'].includes(exitReason)) {
            return { defer: false };
        }
        ...
        const administrativeExitFloor = this._getAdministrativeLiveExitFloor(trade);
        if (!Number.isFinite(administrativeExitFloor)) {
            return { defer: false };
        }
        ...
        if (!hasTwoSidedBook) {
            return {
                defer: true,
                detail: `no_two_sided_book exit=${Number(exitPrice || 0).toFixed(3)} floor=${administrativeExitFloor.toFixed(3)}`
            };
        }

        if (Number.isFinite(spread) && spread > this.liveAdministrativeExitMaxSpread) {
            return {
                defer: true,
                detail: `spread_${spread.toFixed(3)}_gt_${this.liveAdministrativeExitMaxSpread.toFixed(3)}`
            };
        }

        if (Number.isFinite(pnl) && pnl <= 0 && Number.isFinite(exitPrice) && exitPrice < administrativeExitFloor) {
            return {
                defer: true,
                detail: `administrative_loss exit=${exitPrice.toFixed(3)} floor=${administrativeExitFloor.toFixed(3)} pnl=${pnl.toFixed(4)}`
            };
        }
    ...
    getTradeTimingConfig(trade, currentMid, nowMs = Date.now()) {
        ...
        let effectiveStopLoss = trade.stop_loss_price !== null && trade.stop_loss_price !== undefined && Number.isFinite(Number(trade.stop_loss_price))
            ? Number(trade.stop_loss_price)
            : null;
        if (effectiveStopLoss !== null && decayFraction >= decayStartFraction) {
    ...
    calculatePositionSize(signal, wallet) {
        ...
        if (signal.kellyFraction && signal.kellyFraction > 0) {
            ...
            const minSize = isLive
                ? this.getLiveMinPositionSizeUsd(effectiveBalance)
                : 1;
    ...
    canEnterPosition(signal) {
        ...
        const scopedOpenTrades = isLiveSignal
            ? openTrades.filter(t => t.mode === 'live')
            : openTrades.filter(t => t.mode !== 'live');
        if (isLiveSignal) {
        ...
        const categoryCount = scopedOpenTrades.filter(t => t.category === signal.category).length;
        ...
        if (!isShortTTX) {
            ...
            if (signal.direction) {
                const sameDirCount = scopedOpenTrades.filter(
        ...
        if (signal.marketId && signal.marketId.startsWith('GEMI-')) {
            ...
            if (dateMatch) {
                ...
                const sameDateCount = scopedOpenTrades.filter(t => {
        ...
        const alreadyInMarket = scopedOpenTrades.find(t => t.gemini_market_id === signal.marketId);
        if (alreadyInMarket) {
    ...
    async enterPosition(signal) {
        ...
        // GEMI-* crypto: favor Fair Value signals. For canary completion runs,
        // do not hard-block non-FV signals here; later guards still apply.
        if (signal.marketId && signal.marketId.startsWith('GEMI-')) {
            ...
            if (!hasFVSignal) {
                this.logEntryRejection(signal, 'no_fv_signal', 'GEMI_no_fair_value_confirmation');
            }
        }
        ...
        if (this.autonomous15mSession && signal.marketId && signal.marketId.startsWith('GEMI-')) {
            ...
            // Allow non-fair-value signals during canary completion runs.
            // Remaining score/edge/liquidity/risk gates still govern entry quality.

            const baseScoreFloor = entryPolicy.entryThreshold;
            const scoreFloor = this.liveCanaryMode
                ? Math.min(baseScoreFloor, this.liveCanaryMinScore)
                : baseScoreFloor;
            if ((signal.score || 0) < scoreFloor) {
                this.logEntryRejection(signal, 'session_15m_filter', `score_${signal.score || 0}_lt_${scoreFloor}`, {
                    ...
                    liveCanaryMode: this.liveCanaryMode
                });
                return null;
            }
        ...
        const dataMinedGate = this.applyDataMinedRuleGate(signal);
        if (!dataMinedGate.allowed) {
            this.logEntryRejection(signal, 'data_mined_rules', dataMinedGate.reason);
            return null;
        }
        ...
        const effectiveLiveBalance = isLive
            ? (
                this._liveTradableBalance != null
                    ? this._liveTradableBalance
                    : this.getTradableLiveBalance(this._liveBalance)
            ...
            : null;
        const minPositionSize = isLive
            ? this.getLiveMinPositionSizeUsd(effectiveLiveBalance)
            : 1;

        const canaryAutoBumpSize = isLive && this.liveCanaryMode && this.allowSuspendedLiveCanary;
        let effectivePositionSize = positionSize;
        if (effectivePositionSize < minPositionSize) {
            if (canaryAutoBumpSize) {
                effectivePositionSize = minPositionSize;
                this.logger.debug(`Canary size bump: ${positionSize.toFixed(4)} -> ${effectivePositionSize.toFixed(4)}`);
            } else {
        ...
        // NO trade leverage guard: reject very cheap NO entries that create extreme leverage.
        // Canary mode may opt into a lower floor for micro-bootstrap execution.
        const noLeverageFloor = this.liveCanaryMode
            ? this.liveCanaryNoLeverageMinEntryPrice
            : this.noLeverageMinEntryPrice;
        if (signal.direction === 'NO' && entryPrice < noLeverageFloor) {
            this.logger.debug(
            ...
            this.logEntryRejection(
                signal,
                'no_leverage_guard',
                `cost_${entryPrice.toFixed(3)}_lt_${noLeverageFloor.toFixed(2)}`,
                { entryPrice, noLeverageFloor, liveCanaryMode: this.liveCanaryMode }
            );
            return null;
        }

        // Min price guard: reject overly cheap contracts that imply extreme leverage.
        // Default live floor remains $0.10. Canary mode may opt into a lower floor.
        const minEntryPriceFloor = this.liveCanaryMode
            ? this.liveCanaryMinEntryPrice
            : this.liveMinEntryPrice;
        if (entryPrice < minEntryPriceFloor) {
            this.logger.warn(
                `Min price guard: ${signal.direction} ${signal.marketId} ` +
                `entry=$${entryPrice.toFixed(3)} < $${minEntryPriceFloor.toFixed(3)} — too leveraged, rejected`
            );
            this.logEntryRejection(
                signal,
                'min_price_guard',
                `entry_${entryPrice.toFixed(3)}_lt_${minEntryPriceFloor.toFixed(3)}`,
                { entryPrice, minEntryPriceFloor, liveCanaryMode: this.liveCanaryMode }
            );
            return null;
        }
        ...
        const rawContracts = effectivePositionSize / entryPrice;
        // Gemini Predictions API only accepts integer quantities (no fractional contracts)
        let orderQuantity = Math.floor(rawContracts);
        if (orderQuantity < 1) {
            this.logger.debug(`Position too small for 1 contract: $${positionSize.toFixed(2)} / $${entryPrice.toFixed(2)} (raw=${rawContracts.toFixed(2)})`);
            this.logEntryRejection(signal, 'contract_count', 'less_than_1_contract', { entryPrice, rawContracts, positionSize });
            return null;
        }
        ...
        if ((this.gemini.mode === 'live' || this.gemini.mode === 'sandbox') && isRealInstrument) {
            ...
            // Canary mode can opt into a lower floor to allow bootstrap micro-orders.
            const liveScoreFloor = this.liveCanaryMode
                ? Math.min(this.liveMinScore, this.liveCanaryMinScore)
                : this.liveMinScore;
            if ((signal.score || 0) < liveScoreFloor) {
                this.logger.debug(
                    `Live crypto rejected: ${signal.marketId} score=${signal.score} (< ${liveScoreFloor})`
                );
                this.logEntryRejection(
                    signal,
                    'live_min_score',
                    `score_${signal.score}_lt_${liveScoreFloor}`,
                    { entryPrice, liveScoreFloor, liveCanaryMode: this.liveCanaryMode }
                );
                return null;
            }
            ...
            try {
                order = await this.gemini.placeOrder({
                    ...
                    side: 'buy',
                    amount: orderQuantity,  // contracts = dollars / price
            ...
            order = this.gemini.executePaperTrade(
                signal.marketId, signal.direction, effectivePositionSize,
        ...
        for (const trade of openTrades) {
            ...
            if (expiryCheck.timeToExpiry !== null && expiryCheck.timeToExpiry <= 0) {
                ...
                if (isLive) {
                    ...
                    if (stillOnExchange) {
                        this.logger.warn(
                            `LIVE SETTLEMENT WAIT: ${trade.gemini_market_id} expired but exchange still reports position; ` +
                            `deferring close until settlement clears`
                        );
                        continue;
                    }

                    this.logger.warn(
                        `LIVE SETTLEMENT RECONCILE: ${trade.gemini_market_id} expired and no exchange position remains; ` +
                        `closing flat as reconcile_no_exchange instead of assuming worst-case loss`
                    );
                    this.db.closeTrade(trade.id, trade.entry_price, 0, holdTime, 'reconcile_no_exchange');
                    this._recentlyClosed.set(trade.gemini_market_id, Date.now());
                    exits.push({ tradeId: trade.id, reason: 'reconcile_no_exchange', pnl: 0, holdTime });
                    continue;
                }
            ...
            if (isLive) {
                ...
                spread = realPrices ? (realPrices.ask - realPrices.bid) : null;
                hasTwoSidedBook = !!(realPrices && realPrices.hasTwoSidedBook);
            ...
            const hasTakeProfit = trade.take_profit_price !== null
                && trade.take_profit_price !== undefined
                && Number.isFinite(Number(trade.take_profit_price));
            ...
            if (Number.isFinite(effectiveStopLoss) && effectiveStopLoss > trade.entry_price) {
            ...
            if (Number.isFinite(effectiveStopLoss) && trade.direction === 'NO' && trade.entry_price - effectiveStopLoss > 0.15) {
                effectiveStopLoss = trade.entry_price - 0.08;
            }
            ...
            if (hasTakeProfit && exitPrice >= Number(trade.take_profit_price)) {
                exitReason = 'take_profit';
            } else if (!skipStopLoss && Number.isFinite(effectiveStopLoss) && exitPrice <= effectiveStopLoss) {
            ...
            if (isLive && exitReason) {
                const administrativeExit = this._shouldDeferAdministrativeLiveExit({
                    trade,
                    exitReason,
                    exitPrice,
                    pnl,
                ...
                if (administrativeExit.defer) {
                    this.logger.info(
                        `LIVE ADMIN EXIT DEFERRED (${exitReason}): ${trade.gemini_market_id} — ${administrativeExit.detail}`
                    );
                    continue;
                }
            }
