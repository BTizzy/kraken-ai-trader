...
class PolymarketClient {
    constructor(options = {}) {
        ...
        this.minRequestInterval = options.minRequestInterval || 200; // 200ms between requests (5/sec — well under CLOB API limits)
        this.cache = new Map();
        this.cacheTTL = options.cacheTTL || 5000; // 5s cache
        this.priceIndex = new Map(); // condition_id -> { bid, ask, last, spread }
    }
    ...
    /**
     * Fetch all active markets, normalized. Also updates the price index.
     */
    ...
    /**
     * Build/update in-memory price index from normalized markets
     * Used for bulk price lookups without per-market API calls
     */
    updatePriceIndex(normalizedMarkets) {
        for (const m of normalizedMarkets) {
            const yesPrice = m.last_price_yes || 0;
            // Use a deterministic 1¢ half-spread (conservative estimate for liquid Polymarket markets).
            // Removed random noise: for live signal generation we need stable, comparable prices.
            const halfSpread = 0.005; // fixed 0.5¢ half-spread
            this.priceIndex.set(m.market_id, {
                bid: Math.max(0.01, yesPrice - halfSpread),
                ask: Math.min(0.99, yesPrice + halfSpread),
                last: yesPrice,
                spread: halfSpread * 2,
                volume: m.volume || 0,
        ...
        this.logger.debug(`Updated price index: ${this.priceIndex.size} markets`);
    }

    /**
     * Refresh prices from Gamma API for all tracked markets.
     * Much faster than individual CLOB orderbook calls.
     * Returns the number of markets updated.
     */
    async refreshPrices() {
        try {
            const markets = [];
            // Fetch first 200 markets (enough for our 30 matched)
            for (let offset = 0; offset < 200; offset += 100) {
                const batch = await this.getMarkets({ limit: 100, offset });
            ...
            this.logger.warn('Price refresh failed: ' + error.message);
            return 0;
        }
    }

    /**
     * Get cached price for a market by condition ID
     * Returns {bid, ask, last, spread} or null
     */
    getCachedPrice(marketId) {
        return this.priceIndex.get(marketId) || null;
    }
