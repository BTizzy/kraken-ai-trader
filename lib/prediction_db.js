    ...
    initializeTables() {
        this.db.exec(`
            ...
            INSERT OR IGNORE INTO bot_parameters (key, value, min_value, max_value, description) VALUES
                ('entry_threshold', 60.0, 30.0, 90.0, 'Min opportunity score to enter trade'),
    ...
    prepareStatements() {
        ...
        this.stmts = {
            ...
            upsertMatchedMarket: this.db.prepare(`
                INSERT INTO matched_markets (gemini_market_id, polymarket_market_id, kalshi_market_id, event_title, resolution_date, category, match_confidence, manual_override)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                ON CONFLICT(gemini_market_id) DO UPDATE SET
                    polymarket_market_id = excluded.polymarket_market_id,
                    kalshi_market_id = excluded.kalshi_market_id,
