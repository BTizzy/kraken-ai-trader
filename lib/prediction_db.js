...
class PredictionDatabase {
    ...
    migrateSchema() {
        ...
        // V17: Fix phantom live trades + reset parameters for $100 live budget
        try {
            ...
            // Reset parameters for conservative $100 live trading
            this.db.exec(`
                UPDATE bot_parameters SET value = 55.0  WHERE key = 'entry_threshold' AND value < 55.0;
                UPDATE bot_parameters SET value = 10.0  WHERE key = 'max_position_size' AND value > 10.0;
                UPDATE bot_parameters SET value = 20.0  WHERE key = 'max_capital_at_risk_pct' AND value > 20.0;
                UPDATE bot_parameters SET value = 3.0   WHERE key = 'max_concurrent_positions' AND value > 3.0;
                UPDATE bot_parameters SET value = 0.15  WHERE key = 'kelly_multiplier' AND value > 0.15;
                UPDATE bot_parameters SET value = 0.05  WHERE key = 'stop_loss_width' AND value < 0.05;
                UPDATE bot_parameters SET value = -10.0 WHERE key = 'daily_loss_limit' AND value < -10.0;
                UPDATE bot_parameters SET value = 7200  WHERE key = 'max_hold_time' AND value < 7200;
                UPDATE bot_parameters SET max_value = 86400 WHERE key = 'max_hold_time';
            `);
        ...
        // V18: Backtest-optimized parameters
        // Backtest on 97 GEMI-* contracts (19h, BTC/ETH) showed:
        //   - Hold-to-settlement: PnL $5.67 vs $3.94 (V17 baseline), same 3 trades
        //   - Wider SL (10c): no penalty, prevents premature exits
        //   - Edge 5c+H2S+SL10c: $6.01, 100% WR (best pure-WR strategy)
        //   - Cross-platform sim arb: zero opportunity (spreads > divergence)
        //   - Feature: hold_to_settlement flag for expiry-aware holding
        try {
            // Widen stop loss: backtest showed 10c SL matched 5c SL performance
            this.db.exec(`UPDATE bot_parameters SET value = 0.10 WHERE key = 'stop_loss_width' AND value < 0.10`);
            // Increase max hold: backtest confirmed longer holds = better convergence
            this.db.exec(`UPDATE bot_parameters SET value = 14400 WHERE key = 'max_hold_time' AND value < 14400`);
        ...
        this.stmts = {
            ...
            insertEntryRejection: this.db.prepare(`
                INSERT INTO entry_rejections (
                    ...
                    rejection_stage, rejection_reason, entry_price_est, edge_est, mode
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            `),
