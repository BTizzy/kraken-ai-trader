...
class SignalDetector {
    constructor(db, options = {}) {
        ...
        this.fairValueEngine = new FairValueEngine({
            ...
            shortSessionEnabled: options.fairValueShortSessionEnabled === true,
            sessionMinTtxSeconds: options.fairValueSessionMinTtxSeconds,
            sessionMaxTtxSeconds: options.fairValueSessionMaxTtxSeconds,
            shortSessionMinEdge: options.fairValueShortSessionMinEdge
        });
        ...
        // Pending outcome checks: signalId -> { marketId, direction, geminiPrice, timer }
        this.pendingOutcomeChecks = new Map();
    ...
    processMarkets(marketStates) {
        ...
        for (const state of marketStates) {
            ...
            if (state.deribit != null) additionalRefs.deribit = state.deribit;
            ...
            const signal = {
                ...
                actionable: score.total >= this.config.minScore && direction !== null && !onCooldown
                    && gemini?.bid != null && gemini?.ask != null, // Hard liquidity requirement
                on_cooldown: onCooldown,
                timestamp: now
            };

            // Determine rejection reason for non-actionable signals (diagnostic)
            let rejectionReason = null;
            if (!signal.actionable) {
                if (gemini?.bid == null || gemini?.ask == null) rejectionReason = 'no_two_sided_book';
                else if (direction === null) rejectionReason = 'no_direction';
                else if (onCooldown) rejectionReason = 'cooldown';
                else if (score.total < this.config.minScore) rejectionReason = `low_score_${score.total}`;
            }
            ...
            try {
                const signalId = this.db.insertSignal({
                    ...
                    direction: direction,
                    signal_components: JSON.stringify(score.components)
                });

                // Schedule 60s outcome check for actionable signals with direction
                if (signal.actionable && direction && gemini?.last != null) {
                    this.scheduleOutcomeCheck(signalId, marketId, direction, gemini.last);
                }
            } catch (e) {
    ...
    /**
     * Schedule a 60-second outcome check for a logged signal.
     * After 60s, looks up the current Gemini price for the same market
     * and records whether the predicted direction was correct.
     */
    scheduleOutcomeCheck(signalId, marketId, direction, geminiPriceAtSignal) {
        if (!signalId || !marketId) return;

        const timer = setTimeout(() => {
            this.pendingOutcomeChecks.delete(signalId);
            try {
                // Look up current Gemini price from price history
                const geminiHistory = this.priceHistory.get(`${marketId}_gemini`);
                const currentPrice = geminiHistory && geminiHistory.length > 0
                    ? geminiHistory[geminiHistory.length - 1].price
                    : null;

                if (currentPrice == null || geminiPriceAtSignal == null) return;

                const priceMove = currentPrice - geminiPriceAtSignal;
                const directionCorrect =
                    (direction === 'YES' && priceMove > 0) ||
                    (direction === 'NO' && priceMove < 0);

                this.db.updateSignalOutcome(signalId, currentPrice, directionCorrect);
            } catch (e) {
                // Don't crash on outcome check failures
            }
        }, 60000);

        // Prevent timer from keeping Node alive during shutdown
        if (timer.unref) timer.unref();
        this.pendingOutcomeChecks.set(signalId, { marketId, direction, timer });
    }
        ...
        for (const signal of signals) {
            ...
            try {
                const signalId = this.db.insertSignal({
                    ...
                    signal_components: JSON.stringify(signal.models || {})
                });

                // Schedule 60s outcome check
                if (signal.entryPrice != null) {
                    this.scheduleOutcomeCheck(signalId, signal.marketId, signal.direction, signal.entryPrice);
                }
            } catch (e) {
                // Don't fail on logging errors
            }
            ...
            actionable.push({
                ...
                category: 'crypto',
                signalType: 'fair_value',
                pricingModel: signal.pricingModel,
